...
You can also provide an optional JSON property `streamResumeToken` to resume the change stream
...
Example using a resume token:

[source,java]
-------------
from("mongodb:myDb?consumerType=changeStreams&database=flights&collection=tickets&streamResumeToken={'_id':{'_data':'8269DE5... },... }")
    .to("mock:test");
-------------
